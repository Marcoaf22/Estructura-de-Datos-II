...
public class Grafo {    //Grafo dirigido y con peso 
    ...
    public int alcanza(int u, int cost[]) {  //***PREGUNTA 2.
        if (!isVerticeValido(u)) {
            return -1;
        }
        int j = 0;
        do {
            int i = getVertice(u,cost[j]);
            u = i;
            j++;
        } while (u != -1 && j < cost.length);
        return u;
    }

    public int getVertice(int u, int costo) {
        for (int i = 0; i < V[u].length(); i++) {
            if (costo(u, V[u].get(i)) == costo) {
                return V[u].get(i);
            }
        }
        return -1;
    ...
    public int costo(int u, int v) {  //(Esta función es necesaria para resolver la pregunta del examen). Devuelve el peso de la arista u-->v.  Si esa arista no existe, devuelve 0
    ...
    public void addArista(int u, int peso, int v) {  //Crea la arista u-->v
        if (peso <= 0) {
