...
public class Grafo {  //Grafo Dirigido
    ...
    public Grafo(){
     ...
     public void printHojas(int v){  
        if (!isVerticeValido(v, "printHojas"))
            return;  
        
        desmarcarTodos();
        System.out.print("printHojas:");
        printHojas1(v);
        System.out.println();
    }
    
    private void printHojas1(int v){  
        if (V[v].length()==0)
            System.out.print(" "+v);
        marcar(v);
...
//********* Para el marcado de los vértices
    ...
    /*--------------------------------------------------------------------*/
    private boolean alcanzableR(int u,int v){
        boolean sh=u==v;
        marcar(u);
        if (!sh){
    ...
    public boolean alcanzable(int u,int v){
        if (isVerticeValido(v,"alcanzable") && isVerticeValido(u,"alcanzable")){
            desmarcarTodos();
            return alcanzableR(u, v);
        }
        return false;
    }
        private boolean kAlcanzableR(int u,int v,int k){
        boolean sh=u==v && k==0;
        marcar(u);
        if (!sh){
        for (int i=0;i<V[u].length();i++){
