...
public class Grafo {  //Grafo Dirigido
    ...
    public String pathR(int v) {
        String s = " " + v;
        marcar(v);
        int w = primerAdyacenteNoMarcado(v);
        if (w != -1) {
            return s + pathR(w);
        }
        return s;
    ...
    public int primerAdyacenteNoMarcado(int v) {
        for (int i = 0; i < V[v].length(); i++) {
            if (!isMarcado(V[v].get(i))) {
                return V[v].get(i);
            }
        }
        return -1;
    ...
    //para hacer el recorrido en inversa
