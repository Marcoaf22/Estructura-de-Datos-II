...
public class ArbolM {
    ...
    public int sum() {   //PREGUNTA 1
        return sum(raiz);
    }

    public int sum(NodoM p) {
        if (p == null || p.cantHijos() == 0) {
            return 0;
        }
        int suma = 0;
        int aux = 0;
        for (int i = 1; i <= p.cantDatasUsadas()+1; i++) {
            if (p.getHijo(i)!=null && p.getHijo(i).cantHijos() == 0) {
                for (int j = 1; j <= p.getHijo(i).cantDatasUsadas(); j++) {
                    aux += p.getHijo(i).getData(j);
                }
                suma += (aux*i);
            } else {
                suma += sum(p.getHijo(i));
            }
            aux=0;
        }
        return suma;
