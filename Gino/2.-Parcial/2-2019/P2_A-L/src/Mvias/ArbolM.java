...
public class ArbolM {
    ...
    public int getFirstData(int cost) {    //PREGUNTA 1
        return getFristData(raiz, cost);
    }

    public int getFristData(NodoM p, int costo) {
        if (p == null) {
            return -1;
        }
        if (p.getPeso() == costo) {
        ...
        for (int i = 1; i <= p.cantDatasUsadas()+1; i++) {
            if (p.getHijo(i) != null) {
                int s = getFristData(p.getHijo(i), costo - p.getPeso());
                if (s != -1) {
                    return s;
                }
            }
