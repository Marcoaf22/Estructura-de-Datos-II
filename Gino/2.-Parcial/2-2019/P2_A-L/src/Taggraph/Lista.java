...
public class Lista {    //ADT Lista (Ordenada por Data y sin duplicados).
    ...
    public void add(int data, int peso){ //Inserta data/peso a la Lista.
        Nodo Ant = null;
        Nodo p   = L;
        ...
        if (Ant == null){   //x es menor a todos los que están en la Lista (o L==null)
            nuevo = new Nodo(data, peso);
            nuevo.setLink(L);
            L = nuevo;
            n++;
        }
        else
            if (Ant.getData() != data){    //x no está en la lista.  Insertarlo entre Ant y p
                nuevo = new Nodo(data, peso);
                Ant.setLink(nuevo);
                nuevo.setLink(p);
                n++;
    ...
    public int getPeso(int data){    //Devuelve el Peso que acompaña al data.  Si data no existe, devuelve 0.
        Nodo p = exist(data);
        if (p != null)
            return p.getPeso();
